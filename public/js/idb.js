...
request.onupgradeneeded = function(event) {
    ...
    // create an object store (table) called `budget_entry`, set it to have an auto incrementing primary key of sorts 
...
function saveRecord(record) {
    ...
    // access the object store for `budget_entry`
    ...
    if (getAll.result.length > 0) {
      fetch('/api/transaction', {
        ...
        .then(serverResponse => {
          ...
          const transaction = db.transaction(['budget_entry'], 'readwrite');
          // access the budget_entry object store
          const budgetObjectStore = transaction.objectStore('budget_entry');
